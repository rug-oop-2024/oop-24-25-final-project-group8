...
class Dataset(Artifact):
    ...
    def read(self) -> pd.DataFrame:
        """Reads the dataset from its stored format and returns it as a pandas
        DataFrame."""
        ...
        if self.asset_path.endswith(".csv"):
            ...
            # JSON data needs to be loaded and then converted to a DataFrame if it's
            # structured as such
            ...
            csv_string = (
                ...
                else open(self.asset_path, "rb").read().decode()
            )
            ...
            with open(self.asset_path, "rb") as f:
                data = pickle.load(f)
            return (
                pd.DataFrame(data) if isinstance(data, (pd.DataFrame, dict)) else data
            )
        else:
            raise ValueError(
                f"Unsupported data format for asset path: {self.asset_path}"
            )
