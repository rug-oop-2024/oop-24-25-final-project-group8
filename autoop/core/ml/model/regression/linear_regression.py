...
class LinearRegressionModel(Model):
    """
    Wrapper around the LinearRegression from scikit-learn, integrating
    it with the Model interface.
    Provides methods for fitting the model and making predictions, while
    storing model-specific
    ...
    fit_intercept: bool = Field(
        default=True, description="Whether to calculate the intercept for the model."
    )
    normalize: Optional[bool] = Field(
        default=False, description="Whether to normalize the input features."
    )
    copy_X: bool = Field(
        default=True, description="Whether to copy X (input data) or overwrite it."
    )
    ...
    def fit(self, observations: np.ndarray, ground_truth: np.ndarray) -> None:
        ...
        Raises:
            ValueError: If there is a mismatch in dimensions between observations
            and ground truth.
        """
        self._parameters = {}
        ...
        if self.normalize:
            self._model = make_pipeline(
                StandardScaler(),
                LinearRegression(fit_intercept=self.fit_intercept, copy_X=self.copy_X),
            )
        else:
            self._model = LinearRegression(
                fit_intercept=self.fit_intercept, copy_X=self.copy_X
            )
    ...
    def predict(self, observations: np.ndarray) -> np.ndarray:
        ...
        """
        # Validate that the model is fitted and that input dimensions match
        # training dimensions
        self._validate_fit()
