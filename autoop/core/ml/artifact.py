...
class Artifact(BaseModel):
    """
    A class to handle artifacts with associated metadata, supporting multiple data types
    such as DataFrame, bytes, and dictionaries, and allowing saving and loading from
    disk.
    """
    ...
    type: str = Field(
        "unknown type",
        description="Data type of the artifact (dataset, model, pipeline, etc.)",
    )
    version: str = Field("1.0.0", description="Version of the artifact")
    tags: Dict[str, str] = Field(
        default_factory=dict, description="Tags specified by the user for the artifact"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Metadata specified by the user"
    )
    ...
    def generate_id_and_defaults(cls, values: dict[str, Any]) -> dict[str, Any]:
        ...
        values["asset_path"] = (
            values.get("asset_path")
            or f"{values['type']}/{values['name']}_v{values['version']}"
        )
    ...
    def save(self) -> None:
        """
        Saves the artifact's data in `assets/objects`, along with a JSON file in
        `assets/artifacts`.
        """
