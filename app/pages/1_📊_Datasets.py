...
class DatasetsPage:
    def __init__(self) -> None:
        """Initialize DatasetsPage with the AutoMLSystem
        instance and dataset artifacts."""
        # Ensure AutoMLSystem is created only once per session
    ...
    def delete_dataset(self, artifact: Dataset) -> None:
        """
        Deletes the specified dataset's data file and
        metadata file using the registry's delete method.

        Args:
            dataset_id: The ID of the dataset to delete.
        """
        # Use the registry's delete function to remove data and metadata
        self.automl.registry.delete(artifact)
        self.reset_training()

    def display_datasets(self) -> None:
        """Display available datasets with options
        to select, show details, and delete."""
            ...
            for idx, dataset in enumerate(self.artifacts):
                ...
                with st.container():
                    ...
                    with col1:
                        ...
                        st.markdown(
                            f"<div style='{row_bg_color} font-size: 20px; "
                            f"font-style: italic;'> {dataset.name} | "
                            f"{dataset.version}</div>",
                    ...
                    with col2:
                        ...
                        if st.button(
                            "Select Dataset", key=f"btn_select_{idx}_{dataset.name}"
                        ):
                            # if switching selection clear all session state variables
                            if (
                                st.session_state["selected_dataset"]
                                and st.session_state["selected_dataset"] != dataset.id
                            ):
                                self.reset_training()
                    ...
                    with col3:
                        ...
                        if st.button(toggle_text, key=f"btn_show_{idx}_{dataset.name}"):
                            st.session_state[dataset_key] = not st.session_state[
                                dataset_key
                            ]
                            st.rerun()
                    ...
                    with col4:
                        ...
                        if st.button(
                            "Remove Dataset", key=f"btn_delete_{idx}_{dataset.name}"
                        ):
        ...
        if uploaded_file is not None:
            ...
            if st.button("Save Dataset"):
                new_dataset = Dataset.from_dataframe(
                    data=df,
                    name=dataset_name,
                    asset_path=f"{dataset_name}.bin",
                    version=dataset_version,
                )
                self.automl.registry.register(new_dataset)
                st.success(
                    f"Dataset '{dataset_name}' has been registered successfully!"
                )
                st.rerun()
