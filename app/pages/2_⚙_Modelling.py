...
class ModelingPage:
    ...
    def display_dataset(self):
        ...
        if self.selected_dataset:
            ...
            with col1:
                st.markdown(
                    f"<div style='font-size: 20px; font-style: italic;'> {self.selected_dataset.name} | {self.selected_dataset.version}</div>", 
    ...
    def display_model(self):
        ...
        if self.selected_model_name_box:
            ...
            if f"{self.selected_model_name_box}_hyperparameters" not in st.session_state:
                st.session_state[f"{self.selected_model_name_box}_hyperparameters"] = self.model_instance.hyperparameters
        ...
        for param, value in self.hyperparameters.items():
            ...
            if literal_values:
                updated_value = st.selectbox(
                    ...
                    index=literal_values.index(st.session_state.get(f"{self.selected_model_name_box}_{param}", value)),
            ...
            # Determine the widget based on the type and handle None defaults
                ...
                updated_value = st.selectbox(
                    ...
                    index=0 if st.session_state.get(f"{self.selected_model_name_box}_{param}", value) is None else options.index(st.session_state.get(f"{self.selected_model_name_box}_{param}", value)),
                )
            elif isinstance(value, bool):
                # Checkbox for boolean values
                updated_value = st.checkbox(
                    param,
                    value=st.session_state.get(f"{self.selected_model_name_box}_{param}", value),
                    key=f"{self.selected_model_name_box}_{param}",
                )
            elif isinstance(value, int):
                # Render with constraints; if out of range, adjust to the nearest boundary
                input_value = st.session_state.get(f"{self.selected_model_name_box}_{param}", value)
    ...
    def fetch_session_variables(self):
        ...
        self.selected_input_features_objects = st.session_state.get('input_features_objects', [])
        ...
        self.hyperparameters = st.session_state.get(f"selected_model_hyperparameters", {})
    ...
    def display_training_summary(self):
        ...
        <div style="padding: 10px; border: 1px solid #e6e6e6; border-radius: 5px;">
            ...
            <ul style="list-style-type: none; padding-left: 0;">
                ...
                <li><strong>Input Features:</strong> {", ".join(self.selected_input_features)}</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="padding: 10px; border: 1px solid #e6e6e6; border-radius: 5px;">
            ...
            <ul style="list-style-type: none; padding-left: 0;">
                ...
                <ul>
                    { "".join([f"<li>{key}: {value}</li>" for key, value in self.hyperparameters.items()]) }
                </ul>
            </ul>
        </div>
        """, unsafe_allow_html=True)
